...
import { confirmChangesClose, confirmChangesOk } from '@boilerplate/front-end/store/sagas/confirm-changes.saga'
import { confirmChangesSlice } from '@boilerplate/front-end/store/slices/confirm-changes.slice'

interface ConfirmChangesProps {}

const ConfirmChanges: React.FC<ConfirmChangesProps> = () => {
  const dispatch = useAppDispatch()
  const open = useAppSelector(confirmChangesSlice.selectors.open)
  const title = useAppSelector(confirmChangesSlice.selectors.title)
  const description = useAppSelector(confirmChangesSlice.selectors.description)

  const handleCancel = useCallback(() => {
    dispatch(confirmChangesClose())
  }, [])

  const handleConfirm = useCallback(() => {
    dispatch(confirmChangesOk())
  }, [])
  ...
  return (
    <>
      ...
      />
      <div id="confirm-changes-modal" className="pointer-events-none fixed inset-0 z-[10051] overflow-y-auto">
