...
export const ContactMessageForm: React.FC<ContactMessageBaseFormProps> = ({
  firstName,
  lastName,
  email,
  phone,
  ...
  const handleSubmit = useCallback<React.FormEventHandler<HTMLFormElement>>(async (event) => {
    ...
    const { createContactMessageStart } = await import('@boilerplate/front-end/store/sagas/contact-message.saga')

    dispatch(createContactMessageStart({}))
  }, [])
  ...
  return (
    <div className={classes['send-message']}>
      <div className={classes['send-message-box']}>
        ...
        <form action="" className={classes['send-message-box']} onSubmit={handleSubmit}>
